...
> **[당시 기록 — 전부 해소됨]** #3은 §8.4에서 `renderCond`로, #5는 §13에서 확정됐다.
> 나머지 #1(discriminant 자동 추론)·#2(non-terminal 확장)·#4(`partial` 유지)는 **§15**에서
> 닫았다. 셋 다 결론은 "현행 유지"지만, 근거를 남기지 않으면 같은 논의가 재발한다.
...
§5의 다섯 항목 중 셋은 §8.4·§13에서 닫혔고, 남은 셋(#1 discriminant 자동 추론,
#2 non-terminal 확장, #4 `partial` 유지)이 "현재 동작으로 굳어졌을 뿐 명시적으로
닫힌 적 없는" 상태로 남아 있었다. 여기서 닫는다.

**세 건 모두 결론은 "현행 유지"이며 코드 변경은 없다.** 다만 근거를 남기지 않으면
같은 논의가 재발하므로, 무엇을 왜 하지 않기로 했는지를 기록한다.

### 15.1 #1 discriminant 자동 추론 — **하지 않는다**

`renderMatch(state, 'status')`의 키 명시를 유지하고, 추론 오버로드를 **추가하지 않는다.**

당시엔 "키가 여럿/중첩이면 불안정"이라는 직관이었는데, 실제로 타입 레벨에서 확인해보니
불안정한 정도가 아니라 **원리적으로 결정 불가**다. "값이 리터럴 타입인 키"라는 순진한
규칙으로 후보를 뽑으면:

```ts
type Multi = { status: 'a'; kind: 'x' } | { status: 'b'; kind: 'y' }
//  후보 = 'status' | 'kind' — 둘 다 유효한 discriminant라 규칙이 하나를 고를 수 없다

type Trap = { status: 'a'; tag: 'shared' } | { status: 'b'; tag: 'shared' }
//  후보에 'tag'가 포함된다 — 모든 멤버가 공유하므로 아무것도 구분하지 못하는 키다
```

즉 규칙을 정교화해도 (a) 후보가 복수인 경우를 임의로 끊거나 (b) 구분하지 못하는 키를
뽑는 문제가 남는다. 게다가 오버로드를 *추가*하면 실패 시 TS의 오버로드 해상도 에러가
나오는데, 이건 이 라이브러리가 파는 상품(명확한 컴파일 에러)을 정면으로 해친다.

절약되는 것은 인자 하나다. **명시가 정직하다**는 §5 당시의 판단을 그대로 확정한다.

### 15.2 #2 non-terminal 확장(`.map`/`.andThen`) — **하지 않는다 (영구)**

neverthrow의 `.map`은 `Result`에 **특권적인 성공 갈래**가 있기 때문에 의미를 갖는다.
우리가 다루는 건 N개 멤버가 **대등한** 판별 유니온이라, `.map`이 무엇을 변환한다는
것인지 지시 대상 자체가 없다. 차용한 형태가 여기서 끊기는 지점이다.

실용적으로도 빈틈이 없다 — arm이 이미 함수이므로 변환은 arm 안에서 하면 되고,
그 앞의 변환은 JSX 밖에서 하면 된다(§4의 "변환은 JSX 밖, 렌더링만 안"). 따라서
`.map`은 타입 스토리 없는 sugar이고 §8.2 기준 탈락이다. **terminal-only를 영구 확정**한다.

### 15.3 #4 `partial` 유지 — **유지한다**

셋 중 유일하게 실질적 트레이드오프가 있는 항목이라 근거를 자세히 남긴다.

`renderCond`가 생긴 뒤로 `partial`은 **기능상 중복**이다. 같은 일을 두 가지로 쓸 수 있다:

```ts
renderMatch(state, 'status').partial({ error: (s) => <E msg={s.message} /> }, () => <Spin />)
renderCond(state).when({ status: 'error' }, (s) => <E msg={s.message} />).otherwise(() => <Spin />)
```

그럼에도 유지하는 이유:

1. **축이 다르다.** `partial`은 _키 기반 부분집합_, `renderCond`는 *패턴/가드*다. 여러 갈래를
   키로 집을 때 객체 형태가 확실히 짧다 — arm이 2개만 돼도 `.when()` 체인보다 조밀하다.
2. **§14와 정합적이다.** `partial`은 fallback을 **필수로 요구**한다. 즉 "생략 가능한 갈래"가
   아니라 "명시적으로 좁힌 갈래 + 명시적 fallback"이라, `renderShow`/`renderEach`에서
   두 arm을 필수로 만든 결정과 같은 기조다.
3. **비용이 거의 없다.** `arms`와 조회 경로(`armFor`)를 공유하는 몇 줄이고, 이미 테스트와
   문서가 있다. 순수주의만을 이유로 동작하는 기능을 걷어내는 건 사용자 이득이 0이다.

> **함께 검토했으나 하지 않기로 한 것**: 터미널 이름이 `.arms()`(대상)와 `.partial()`(모드)로
> 축이 어긋난다. `.partialArms()` 같은 대안은 더 나빠서, 개명 없이 그대로 둔다. 기록만 남긴다.
