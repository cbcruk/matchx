...
export const rows = renderEach(users, (u, i) => `${i}:${u.name}`)

/* --- positive: a Map is iterated as [key, value] entries ------------------ */

export const entries = renderEach(byId, ([id, u]) => `${id}=${u.name}`)

/* --- positive: a Set is iterated by element ------------------------------- */

export const set = renderEach(ids, (n) => n * 2)

/* --- negative: the render cannot read fields the item does not have -------- */

export const wrongField = renderEach(
  users,
  (u) =>
...
export const wrongIndex = renderEach(users, (u, i) =>
...
export const doubled: number[] | null = renderEach([1, 2, 3], (n) => n * 2)

/* --- positive: the fallback may differ in type from the item render -------- */

export const withFallback: string[] | number = renderEach(
  ['a'],
  (s) => s,
  () => 0,
)
...
export const notIterable = renderEach(42, (n) => n)
